...
def get_lectura_cuant():
    try:
        ...
        ront_scan=lectura[300:300]
        #print (right_scan)
        #print (left_scan)
        #print (ront_scan)
        sd,si=0,0
        if np.mean(left_scan)< 2.1: si=1 
      
        if np.mean(right_scan)< 2.1: sd=1
        
        msg1 = f'letf {np.mean(left_scan)}'
        print (msg1)
        msg2 = f'right {np.mean(right_scan)}'
        print (msg2)
